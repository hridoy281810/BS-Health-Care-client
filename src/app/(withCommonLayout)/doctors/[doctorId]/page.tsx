...
const DoctorsProfilePage = async ({ params }: PropTypes) => {
   const res = await fetch(`http://localhost:5000/api/v1/doctor/${params.doctorId}`);
   console.log(params.doctorId);
   const { data: doctor } = await res.json();
   console.log(doctor?.schedules);
   
   const res2 = await fetch(`http://localhost:5000/api/v1/specialties`);
   const { data: specialtiesData } = await res2.json();
   const specialties = doctor?.doctorSpecialties?.map((ds: any) => ({
    specialtiesId:ds?.specialtiesId}));
   const sortedSpecialties = specialties?.map((specialty: any) => {
